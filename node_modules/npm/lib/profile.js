...
function get(args) {
    ...
    Object.keys(info).filter((k) = > !(k in cleaned)
).
    forEach((k) = > {cleaned[k] = info[k] || ''}
)
    delete cleaned.tfa
...
function enable2fa(args) {
...
)
    {
        ...
        throw new Error('Your registry ' + conf.registry + 'does not seem to support bearer tokens. Bearer tokens are required for two-factor authentication')

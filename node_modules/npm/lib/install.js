    ...
    if (this.packageLockOnly) {
        ...
        if (getSaveType()) {
            postInstallSteps.push(
                ...
                [this, (next) = > {computeMetadata(this.idealTree);
            next()
        }],
...
Installer.prototype.pruneIdealTree = function (cb) {
    ...
    if (!this.idealTree.hasRequiresFromLock && this.idealTree.children.some((n) = > n.fakeChild))
    return cb()
...
Installer.prototype.printInstalled = function (cb) {
    ...
    if (!this.idealTree.error && this.idealTree.removedChildren) {
        ...
        this.idealTree.removedChildren.forEach((r) = > {
            if(diffs.some((d) = > d[0] === 'remove' && d[1].path === r.path
    ))
        return
...
Installer.prototype.archyDebugTree = function (tree) {
    ...
    function expandTree(tree) {
        ...
        return {
            ...
            nodes: tree.children.filter((tree) = > {return !seen.has(tree) && !tree.removed}
    ).
...
Installer.prototype.archyDebugLogicalTree = function (tree) {
    ...
    function expandTree(tree) {
        ...
        return {
            label: packageId(tree),
            nodes: tree.requires.filter((tree) = > {return !seen.has(tree) && !tree.removed}
    ).
        sort(byName).map(expandTree)
    }
    }
